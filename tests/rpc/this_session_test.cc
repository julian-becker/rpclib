...
TEST_F(this_session_test, post_exit) {
    ...
    EXPECT_EQ(c.get_connection_state(), client<rpc::backend::msgpack_client>::connection_state::disconnected);
}
...
TEST_F(this_session_test, post_exit_specific_to_session) {
    ...
    rpc::client<rpc::backend::msgpack_client> c("127.0.0.1", test_port);
    rpc::client<rpc::backend::msgpack_client> c2("127.0.0.1", test_port);
    c2.call("exit", false);
    c.call("exit", true);
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    auto f = c.async_call("exit");
    ...
    EXPECT_EQ(f.wait_for(std::chrono::milliseconds(50)), std::future_status::timeout);
    EXPECT_EQ(c.get_connection_state(), client<rpc::backend::msgpack_client>::connection_state::disconnected);
    EXPECT_EQ(c2.get_connection_state(), client<rpc::backend::msgpack_client>::connection_state::connected);
}
